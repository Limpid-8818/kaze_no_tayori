  ...
  '\$value': {
    ...
    'stops': [
      for (final (i, c) in g.colors.indexed)
        {'color': _hex(c), 'position': _trimNum(g.stops![i])},
    ],
  },
  '\$description': description,
};
...
String _hex(Color c) {
  ...
  return a == 0xFF
      ? '#$rgb'
      : '#$rgb${a.toRadixString(16).toUpperCase().padLeft(2, '0')}';
}
...
void main() {
  test('导出 W3C DTCG 令牌 JSON（单一事实来源反生成）', () {
    final json = <String, Object>{
      ...
      'natsu': {
        'meta': {
          ...
          'concept': 'Summer Epistolary · 光在纸上 — 环境是夏日天空，纸只在「信」的时候出现',
          'format': 'W3C Design Tokens Community Group (DTCG)',
          'generated': '2026-08-20',
          'generator': 'test/tool/export_design_tokens_test.dart（单一事实来源：lib/src/tokens/）',
          'layers': {
        ...
        'color': {
          ...
          'sky-horizon': _color(
            _hex(NatsuColors.skyHorizon),
            '地平线 — 暖白（渐变下端，stops 0.72）',
          ),
          ...
          'ink-faint': _color(
            _hex(NatsuColors.inkFaint),
            '淡墨 — caption（仅非关键小字）',
          ),
          // 点缀层
          'sky-blue': _color(_hex(NatsuColors.skyBlue), '夏空蓝 — 交互主色（链接/选中/聚焦）'),
          'sunlight-yellow': _color(
            _hex(NatsuColors.sunlightYellow),
            '阳光黄 — 稀有高光；永不作文字色',
          ),
          'coral-stamp': _color(
            _hex(NatsuColors.coralStamp),
            '珊瑚 — 邮戳/邮票/旅行标记（配给色）',
          ),
          // 状态层
          'hover-overlay': _color(_hex(NatsuColors.hoverOverlay), '悬停 — 墨蓝 4%'),
          'pressed-overlay': _color(
            _hex(NatsuColors.pressedOverlay),
            '按压 — 墨蓝 8%',
          ),
          'disabled-content': _color(
            _hex(NatsuColors.disabledContent),
            '禁用内容 — 32%',
          ),
        ...
        'fontFamily': {
          ...
          'hw': _fontFamily(
            'LXGWWenKai',
            NatsuFontFamilies.hwFallback,
            '手写 — 内容物的声音',
          ),
          'quote': _fontFamily(
            'NotoSerifSC',
            NatsuFontFamilies.quoteFallback,
            '引用 — 铅字（系统中唯一衬线场景）',
          ),
          'seal': _fontFamily(
            'MaShanZheng',
            NatsuFontFamilies.sealFallback,
            '刻印 — 品牌位',
          ),
        },
        ...
        'typography': {
          ...
          'hw-seal': _typographyFrom(
            NatsuTypography.hwSeal,
            '刻印 — MaShanZheng 珊瑚',
          ),
          'quote': _typographyFrom(
            NatsuTypography.quoteSerif,
        ...
        'spacing': {
          ...
          'control-hit-target': _dimension(
            NatsuSpacing.controlHitTarget,
            '控件最小命中区',
          ),
          ...
          // 编辑刻度（展示页容器）
          'page-padding': _dimension(
            NatsuSpacingEditorial.pagePadding,
            '页面左右余白',
          ),
          'section-gap': _dimension(
            NatsuSpacingEditorial.sectionGap,
            '大章节间（間）',
          ),
        ...
        'border': {
          ...
          'input': {
            '\$type': 'border',
            '\$value': {'style': 'solid', 'width': '1px', 'color': '#D5D0C4'},
            '\$description': '输入线（稍深）',
          },
        },

        'shadow': {
          'paper-resting': _shadow(NatsuShadows.paperResting, '纸面静置 — 纸从天空浮起'),
        ...
        'motion': {
          ...
          'toast-duration': _duration(
            NatsuMotion.toastDuration.inMilliseconds,
            '轻提示停留（非动效时长）',
          ),
    ...
    expect(
      (colors['ink-blue'] as Map<String, dynamic>)['\$value'],
    ...
    final duskSunny =
        (weather['dusk-sunny'] as Map<String, dynamic>)['\$value']
            as Map<String, dynamic>;
    expect(
      (duskSunny['stops'] as List<dynamic>).length,
      4,
      reason: '夕·晴是 4 stops 琥珀带结构',
    );
    print(
      '✓ 导出 ${file.path}（${(file.lengthSync() / 1024).toStringAsFixed(1)} KB）',
    );
  });
}
