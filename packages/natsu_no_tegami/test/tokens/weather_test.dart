...
void main() {
  ...
  test('回归基准：昼·晴 == v2 默认天空（同一 const，colors/stops 逐项相等）', () {
    ...
    expect(identical(base, NatsuColors.skyGradient), isTrue,
  ...
  test('夕的 4 stops：琥珀带占中腹部（暖色止于 0.78，非底部全暖）', () {
    for (final weather in NatsuWeather.values) {
      final stops = NatsuWeatherLight.of(weather, NatsuTimeOfDay.dusk)
          .gradient
          .stops!;
      expect(stops.length, 4,
          reason: '$weather · 夕必须是 4 stops 的琥珀带结构');
    }
    ...
    for (final time in [
      ...
      for (final weather in NatsuWeather.values) {
        expect(
            NatsuWeatherLight.of(weather, time).gradient.stops!.length, 2,
