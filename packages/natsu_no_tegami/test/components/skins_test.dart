...
void main() {
  group('注册表完整性', () {
    test('每槽数量符合「代表资产」定位（防缩水防膨胀）', () {
      ...
      expect(NatsuSkins.bySlot(SkinSlot.postcard), isEmpty,
    ...
    test('ID 全局唯一且匹配 <slot>.<motive>-<数字> 规范', () {
      ...
      for (final id in ids) {
        ...
        expect(NatsuSkins.byId(id)!.slot.name, id.split('.').first,
    ...
    test('painter 全部无状态（shouldRepaint 恒 false）', () {
      for (final a in NatsuSkins.all) {
        expect(a.painter.shouldRepaint(a.painter), isFalse,
  ...
  group('coral 配给纪律', () {
    test('邮戳槽全珊瑚（盖章油墨统一）', () {
      for (final a in NatsuSkins.bySlot(SkinSlot.postmark)) {
        expect(a.ink, NatsuColors.coralStamp,
            reason: '${a.id} 邮戳油墨必须统一珊瑚');
      }
    });
    ...
    test('装饰槽禁珊瑚/错误/顶光浅色（配给 + 可见性）', () {
      ...
      for (final a in NatsuSkins.bySlot(SkinSlot.decor)) {
        expect(a.ink, isNot(anyOf(forbidden)),
            reason: '${a.id} 装饰用色越界');
      }
    });
  });
  ...
  group('消费端视觉契约', () {
    testWidgets('Postmark 无 emblem = 纯文字（零回归；地名弧排逐字）',
        (tester) async {
    ...
    testWidgets('Postmark 有 emblem：接口保留（暂不参与构图），文字仍在',
        (tester) async {
      await tester.pumpWidget(_wrap(const Postmark(
        place: '鎌倉',
        date: '2026.08.20',
    ...
    testWidgets('StampPiece + SkinMotive：锯齿边与面值不破坏', (tester) async {
      await tester.pumpWidget(_wrap(const StampPiece(
        seedId: 'skin-stamp-1',
    ...
    testWidgets('SkinMotive 未注册 id 渲染为空不抛', (tester) async {
      await tester.pumpWidget(
          _wrap(const SkinMotive('decor.nowhere-99')));
