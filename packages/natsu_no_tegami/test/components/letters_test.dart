...
void main() {
  testWidgets('LetterPaper 渲染正文/meta/计数句', (tester) async {
    await tester.pumpWidget(_wrap(LetterPaper(
  ...
  testWidgets('Postmark size 参数：呈现尺寸 == size（viewBox 等比）',
      (tester) async {
    await tester.pumpWidget(
  ...
  testWidgets('StampPiece 比例锁 0.8：宽高恒定，宽可随意缩放', (tester) async {
    await tester.pumpWidget(
        _wrap(const Row(children: [StampPiece(seedId: 's1'), StampPiece(seedId: 's2', width: 40)])));
    expect(StampPiece.aspectRatio, 0.8);

    final s64 = tester.getSize(find.byWidgetPredicate(
        (w) => w is StampPiece && w.seedId == 's1'));
    final s40 = tester.getSize(find.byWidgetPredicate(
        (w) => w is StampPiece && w.seedId == 's2'));
  ...
  testWidgets('DeskScene 入场后全部可见', (tester) async {
    await tester.pumpWidget(_wrap(DeskScene(
      height: 640,
      animateIn: true,
      children: [
        const Positioned(left: 20, top: 20, child: StampPiece(seedId: 'd1')),
        const Positioned(
          left: 120,
          top: 40,
