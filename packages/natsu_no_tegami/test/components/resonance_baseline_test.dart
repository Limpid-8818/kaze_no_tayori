...
void main() {
  group('NatsuResonance 字体基线（修复回归锁）', () {
    testWidgets('行动字与句子活在同一段落——基线由排版器统一', (tester) async {
      ...
      String leafText(TextSpan s) =>
          (s.children ?? <InlineSpan>[])
              .whereType<TextSpan>()
              .map(leafText)
              .followedBy([s.text ?? ''])
              .join();
      ...
      expect(
        texts.whereType<String>().any((t) => t.contains('陌生人也曾')),
        isTrue,
      );
    ...
    testWidgets('段落内基线几何：两个 span 的基线 y 一致（Ahem 下也成立）',
        (tester) async {
  ...
  group('导出对话框布局约束（溢出修复回归锁）', () {
    testWidgets('预览图限高：竖长图不把说明/按钮顶出屏幕', (tester) async {
      ...
      const maxW = 360.0;
      final fittedH =
          imgH * (maxW / imgW) > maxHeight ? maxHeight : imgH * (maxW / imgW);
