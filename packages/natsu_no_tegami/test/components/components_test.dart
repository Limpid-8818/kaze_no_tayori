...
void main() {
  group('NatsuButton', () {
    testWidgets('四变体渲染且可点击', (tester) async {
      ...
      await tester.pumpWidget(_wrap(Column(
        ...
        children: [
          ...
          NatsuButton(
              variant: NatsuButtonVariant.ghost, onPressed: () {}, child: const Text('继续旅行')),
          NatsuButton(
    ...
    testWidgets('禁用态不响应点击', (tester) async {
      await tester.pumpWidget(_wrap(
        const NatsuButton(onPressed: null, child: Text('禁用')),
      ));
  ...
  group('NatsuCard / NatsuQuote / NatsuSeal / MetaLine', () {
    testWidgets('组合渲染', (tester) async {
      await tester.pumpWidget(_wrap(NatsuCard(
  ...
  group('NatsuSwitch', () {
    ...
    testWidgets('禁用不触发', (tester) async {
      var fired = false;
      await tester.pumpWidget(_wrap(
          NatsuSwitch(value: false, onChanged: (v) => fired = true, enabled: false)));
  ...
  group('NatsuCheckbox', () {
    testWidgets('点击 false→true，选中块 = 夏空蓝 + 勾存在', (tester) async {
      var value = false;
      await tester.pumpWidget(
          _wrap(NatsuCheckbox(value: value, onChanged: (v) => value = v ?? false)));
    ...
    testWidgets('禁用不触发', (tester) async {
      var fired = false;
      await tester.pumpWidget(_wrap(NatsuCheckbox(
          value: false, onChanged: (v) => fired = true, enabled: false)));
  ...
  group('NatsuRadio', () {
    testWidgets('点击第二项回调其值', (tester) async {
      ...
      await tester.pumpWidget(_wrap(Column(
        ...
        children: [
          NatsuRadio<String>(
              value: 'yoru', groupValue: null, onChanged: (v) => picked = v),
          NatsuRadio<String>(
              value: 'asa', groupValue: null, onChanged: (v) => picked = v),
        ],
      )));
    ...
    testWidgets('选中态内点 = 夏空蓝', (tester) async {
      ...
      final decorated = tester
          .widget<DecoratedBox>(find.descendant(
              of: find.byType(AnimatedScale), matching: find.byType(DecoratedBox)))
  ...
  group('NatsuSlider', () {
    testWidgets('初始旋钮位置反解 == 2f-1，拖动更新值', (tester) async {
      ...
      await tester.pumpWidget(_wrap(StatefulBuilder(
        builder: (context, setState) => SizedBox(
          width: 320,
          child: NatsuSlider(value: value, onChanged: (v) => setState(() => value = v)),
        ),
      )));
      await tester.pumpAndSettle();
      ...
      final knobs = find.descendant(
          of: find.byType(NatsuSlider), matching: find.byType(Align));
    ...
    testWidgets('禁用无响应', (tester) async {
      ...
      await tester.pumpWidget(_wrap(SizedBox(
        width: 320,
        child: NatsuSlider(value: 0.5, onChanged: (v) => fired = true, enabled: false),
      )));
  ...
  group('NatsuSpinner', () {
    testWidgets('渲染邮戳环且 dispose 无错', (tester) async {
      ...
      expect(
          find.descendant(
              of: find.byType(NatsuSpinner), matching: find.byType(CustomPaint)),
          findsOneWidget);
    });
  });
  ...
  group('showNatsuSheet', () {
    testWidgets('把手存在 + barrier 可关', (tester) async {
      ...
      final handleBox = tester.renderObject<RenderBox>(find.byWidgetPredicate(
          (w) =>
              w is Container &&
...
Color? _switchTrackColor(WidgetTester tester) {
  final track = tester.widget<AnimatedContainer>(
      find.descendant(of: find.byType(NatsuSwitch), matching: find.byType(AnimatedContainer)));
