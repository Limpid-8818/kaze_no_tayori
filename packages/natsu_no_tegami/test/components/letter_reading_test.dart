...
void main() {
  group('LetterBlock 序列化', () {
    test('TextBlock/PhotoBlock toJson/fromJson round-trip', () {
      ...
      final restored = [
        for (final b in blocks) LetterBlock.fromJson(b.toJson()),
      ];
  ...
  group('validateLetterFlow', () {
    test('0–3 图合法', () {
      expect(validateLetterFlow([const TextBlock('a')]), isNull);
      expect(
        validateLetterFlow([
          const TextBlock('a'),
  ...
  group('图文交替流渲染', () {
    testWidgets('文-图-文流：照片夹在两段正文之间（y 坐标居中）', (tester) async {
